...
determinant_fun = function(cond = T, ifsimu  = T, n_simu = 1){
  ...
  if(cond){
    ...
    # Fit the model
    fit <- stan(file = 'F5D_VOC3.stan', data = stan_data, 
...
if(F){
  ...
  save(fitlist, plot_data, file = 'voc3.rdata')  
}
...
p = ggplot() +
  ...
  scale_y_continuous(breaks = 0:2*10^4, labels = 0:2) +
...
p
pdf(paste0("Output/F5D_voc3.pdf"), width = 2.5, height = 1.8)
print(p)
dev.off()
...
p = ggplot() +
  ...
  theme(legend.position = 'none',
        ...
        legend.key.size = unit(0.2, units = 'cm'),
        panel.grid.minor = element_blank())
pdf(paste0("Output/F5D_voc3_mainfig.pdf"), width = 1.8, height = 1.3)
print(p)
dev.off()
...
pdf(paste0("Output/F5D_voc3_beta.pdf"), width = 2.5, height = 1.2)
print(p3)
print(p4)
dev.off()
...
if(F){
  pdf(file = 'Output/trace_voc3.pdf', 
      width = 5, height = 3.4)
  for (i in 1:21) {
