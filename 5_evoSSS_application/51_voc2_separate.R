...
update_fun <- function(pars, states_old, N, n) {
  S <- states_old[,1:n]
  I <- states_old[,(n+1):(2*n)]
  Onset <- states_old[,(2*n+1):(3*n)]
  ...
  S_new <- S
  for (i in 1:n) {
    S_new[,i] <- S_new[,i] - betas[i] * S[,i] * I[,i] / N
  }
  
  I_new <- matrix(0, nrow(states_old), n)
  for (i in 1:n) {
    I_new[,i] <- I[,i] + betas[i] * S[,i] * I[,i] / N - gamma * I[,i] 
  }
  
  Onset_new <- matrix(0, nrow(states_old), n)

  for (i in 1:n) {
    Onset_new[,i] <- betas[i] * S[,i] * I_new[,i] / N 
  }
...
determinant_fun = function(cond = T, ifsimu  = T, n_simu = 1){
  ...
  if(cond){
    ...
    # Fit the model
    fit <- stan(file = 'VOC2_separate.stan', data = stan_data, 
