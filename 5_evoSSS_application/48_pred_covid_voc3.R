...
determinant_fun = function(cond = F, ifsimu  = F, n_simu = 1, predj = 1){
  ...
  # Create a dataframe with all dates
  d0 <- expand.grid(date = as.Date('2020-06-20') + 1:10, 
                    V = unique(df$V))
  d1 <- d0 %>%
    left_join(df, by = c("date", "V")) %>%
    mutate(count = ifelse(is.na(count), 0, count))
  ...
  # save(fitlist, file = 'voc3_chain4.rdata')
...
df3 = dfall[dfall$color == 3,]
fit3 <- lm(log10(y.y) ~ log10(y.x), data = df3)
slope3 <- coef(fit3)[2]
intercept3 <- coef(fit3)[1]
adj_r2_3 <- summary(fit3)$adj.r.squared
coord3 = log10(c(min(df3[, c('y.x','y.y')]), max(df3[, c('y.x','y.y')])))

p1 = ggplot(data = df1, aes(x = log10(y.x), y =log10(y.y),
                       group = x, color =x)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = slope1, intercept = intercept1) +
  annotate("text", x = 2.3, y = -0.4, size = 3,
...
p2 = ggplot(data = df2, aes(x = log10(y.x), y =log10(y.y),
  ...
  annotate("text", x = 2.85, y = -0.8, size = 3,
           label = paste0("y = ", round(slope2, 2), "x + ", round(intercept2, 2), # 1x
                          "\nR² = ", round(adj_r2_2, 3))) +
  xlab(expression(log[10](Expected))) + ylab(expression(log[10](Predicted))) +
  theme_bw() +
...
p3 = ggplot(data = df3, aes(x = log10(y.x), y =log10(y.y),
                       group = x, color =x)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = slope3, intercept = intercept3) +
  annotate("text", x = 3.5, y = 0.6, size = 3,
           label = paste0("y = ", round(slope3, 2), "x + ", round(intercept3, 2), # 1x
                          "\nR² = ", round(adj_r2_3, 3))) +
  xlab(expression(log[10](Expected))) + ylab(expression(log[10](Predicted))) +
  theme_bw() +
  coord_cartesian(xlim = coord3, ylim = coord3) +
...
pdf(paste0("Output/pred_covid_voc3.pdf"), width = 3, height = 2)
print(p1)
print(p2)
print(p3)
dev.off()
