...
full_dates <- as.Date('2020-06-30') + 1:890
...
dfvoc2 = merged_df[merged_df$V != 'Omicron',]
dfvoc2 = dfvoc2 %>% group_by(date) %>%
  summarise(count = sum(count))
dfvoc2$V = 'Non Omicron'

dfvoc2 = rbind(dfvoc2, merged_df[merged_df$V == 'Omicron',colnames(dfvoc2)])
dfvoc2$V = factor(dfvoc2$V, levels = c('Omicron', 'Non Omicron'))

values = brewer.pal(n=4, name = "Spectral")[1:2]
values = c(values[1], '#41424c')
show_col(values)
p1 = ggplot(data = dfvoc2, 
            aes(x = date, y = count, 
                group = V, color = V)) +
  geom_line() +
  scale_color_manual(name = '',
                     breaks = rev(levels(dfvoc2$V)),
                     values = rev(values)) +
  theme_bw() +
  ...
  xlab('') + ylab('Cases') + 
  coord_cartesian(xlim = c(as.Date('2021-07-01'), as.Date('2022-09-01')),
                  ylim = c(1,2*10^4)) +
...
p1
pdf(paste0("Output/voc2_curve.pdf"), width = 2.5, height = 1.6)
print(p1)
dev.off()
...
data2 = dfvoc2 %>%
  ...
  as.data.frame()
data2$group = factor(data2$V, levels = c('Omicron','Non Omicron'))
...
p2 = ggplot() +
  ...
  coord_cartesian(xlim = c(as.Date('2021-01-01'), as.Date('2022-10-01')),
                  ylim = c(0,1)) +
...
pdf(paste0("Output/voc2_prevalence.pdf"), width = 2.5, height = 1.2)
print(p2)
dev.off()


values = brewer.pal(n=4, name = "Spectral")
p1 = ggplot(data = merged_df[merged_df$V != 'Omicron',], 
            aes(x = date, y = count, 
                group = V, color = V)) +
  geom_line() +
  scale_color_manual(name = '',
...
pdf(paste0("Output/voc3_curve.pdf"), width = 2.5, height = 1.6)
print(p1)
dev.off()


data2 = merged_df[merged_df$V != 'Omicron',] %>%
  group_by(date) %>%
  mutate(p = count/sum(count)) %>%
  as.data.frame()
data2$group = factor(data2$V, levels = c('Delta','Alpha','D614G'))
...
pdf(paste0("Output/voc3_prevalence.pdf"), width = 2.5, height = 1.2)
print(p2)
dev.off()
