...
df2 = fread('F6D_WHO-COVID-19-global-data.csv')
df2[is.na(df2)] = 0
...
if(F){
  ...
  pdf(paste0("Output/S8D_fc_flu.pdf"), width = 4.5, height = 1.4)
  print(p1)
  dev.off()
  ...
  for (j in 1:59) {
    if(!ifsimu)print(j)
    {
      ...
      if(cond){
        ...
        # Fit the model
        fit <- stan(file = 'F6D_flu_covid.stan', data = stan_data, 
                    iter = 3000, chains = 1, warmup = 2000,
                    verbose = TRUE)
        fitlist[[j+1]] = fit
      }
...
if(F){
  save(fitlist, Onsets_mat_list, simu_Onset, file = 'F6D_fc.rdata')
...
pdf(paste0("Output/F6D_fc.pdf"), width = 4.5, height = 1.8)
print(p)
dev.off()
...
p3 = ggplot() +
  ...
  theme(plot.margin = unit(c(0.3,0.6,0,0), "cm"),
        panel.grid.minor = element_blank())
pdf(paste0("Output/F6D_fc_beta.pdf"), width = 4.3, height = 1.2)
print(p3)
dev.off()
...
p3 = ggplot() +
  ...
  theme(plot.margin = unit(c(0.3,0.6,0,0), "cm"),
        panel.grid.minor = element_blank())
pdf(paste0("Output/F6D_fc_beta_mainfig.pdf"), width = 1.8, height = 1.2)
print(p3)
dev.off()
