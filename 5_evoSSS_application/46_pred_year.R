...
fitlist = list()
determinant_fun = function(cond = F, ifsimu  = F, n_simu = 1, pred = 15){
  # cond = T; ifsimu  = F; n_simu = 1
  # cond = F; ifsimu  = T;n_simu = 1; pred = 13; pred = 25
  n = 2
  poolday = 30
  nday = 100
  ...
  for (j in 1:(pred+24)) {
    {
      ...
      if(j==pred){
        posterior1 = rstan::extract(fitlist[[j]])
        posterior2 = rstan::extract(fitlist[[j-1]])
        posterior3 = rstan::extract(fitlist[[j-2]])

        pars_last[2] = median(c(posterior1$beta1, posterior2$beta1, posterior3$beta1))
        pars_last[3] = median(c(posterior1$beta2, posterior2$beta2, posterior3$beta2))
      }
      if(j >= pred){
        posterior = rstan::extract(fitlist[[j+1-12]])
        pars_last[1] = posterior$contact[n_simu]
        if(ifsimu) pars_last[1] = mean(posterior$contact)
      }else{
        posterior = rstan::extract(fit)
        pars_last = c(mean(posterior$contact), 
                      mean(posterior$beta1),
  ...
  if(F){
    ggplot() +
      ...
      geom_line(data = data[data$date < as.Date('2017-08-01') + months(pred),], 
                aes(x = date, y = y+1, group = group, color = group)) +
      geom_line(data = data[data$date >= as.Date('2017-08-01') + months(pred)&
                  data$date < as.Date('2017-08-01') + months(pred+12),], 
                aes(x = date, y = y+1, group = group, color = group),
                linetype = 'dashed') +
      # scale_y_continuous(transform = 'log') +
...
if(F){
  ...
  for (n_simu in 1:100) {
    print(n_simu)
    data = determinant_fun(cond = F, ifsimu  = T, n_simu = n_simu, pred = 15)
...
p = ggplot() +
  ...
  geom_line(data = plot_data[plot_data$date < as.Date('2017-08-01') + months(pred),], 
...
pdf(paste0("Output/pred_year.pdf"), width = 4.4, height = 1.6)
print(p)
dev.off()

preddate = as.Date('2017-08-01') + months(pred)
preddate2 = as.Date('2017-08-01') + months(pred+24)
plot_data = plot_data[plot_data$date >= preddate &
                        plot_data$date < preddate2,]
...
dfob2 = dfob[dfob$x >= preddate & dfob$x < preddate2,]

r =c()
for (i in 1:24) {
  k = ks.test(dfob2$y[c(1:i,1:i+24)], plot_data2$Fitted2[c(1:i,1:i+24)])
  r = c(r,k[['p.value']])
}
rdata = data.frame(x = dfob2$x[1:(nrow(dfob2)/2)], p = r)
p2 = ggplot(data = rdata,
           aes(x = x, y = p)) +
  ...
  theme(legend.position = "none",
        panel.grid.minor = element_blank(),
        plot.margin = margin(0.2,0.6,0,0.2, "cm"))
p2
pdf(paste0("Output/pred_year_kp.pdf"), width = 2.5, height = 1)
print(p2)
dev.off()
