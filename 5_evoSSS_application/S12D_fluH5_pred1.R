...
plot_data <- data.frame(
  x = data$x,
  group = data$color,
  Fitted = apply(simu_Onset, 1, median, na.rm=T),
  LowerCI = ci_lower,
  UpperCI = ci_upper
)
...
pdf(paste0("Output/S12D_fluH5_pred1.pdf"), width = 4, height = 1.6)
print(p)
dev.off()
...
p2 =  ggplot() +
  ...
  coord_cartesian(xlim = c(as.Date('2021-09-01'), as.Date('2025-05-31')))
p2
pdf(paste0("Output/S12D_fluH5_pred1_prevalence.pdf"), width = 4, height = 1.2)
print(p2)
dev.off()
