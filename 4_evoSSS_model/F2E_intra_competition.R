...
load('evoSIR/F2E_intra_competition.rdata')
df_all = read.csv("../2_Experiment/S8A_vcf_231228.csv")
sampleid = read.csv("../2_Experiment/S5C_sampleid.csv")
...
if(F){
  fit <- stan(file = 'F2E_intra_competition.stan', data = stan_data, 
              iter = 3000, chains = 1, 
...
pdf(file = 'Output/F2E_intra_competition.pdf', width = 2, height = 1.4)
print(p1)
print(p2)
dev.off()
...
pdf(file = 'Output/S8E_intra_competition_cycle.pdf', width = 2, height = 1.4)
print(p5)
print(p6)
dev.off()
