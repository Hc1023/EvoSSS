...
determinant_fun = function(cond = T, ifsimu  = T, n_simu = 1, sampling = T){
  ...
  for (j in 1:24) {
    ...
    
    {
      ...
      Mobility_matrix <- diag(rep(mobility, 30) )
      ...
      if(cond){
        ...
        # Fit the model
        fit = stan(file = 'evoSSS/capacity.stan', data = stan_data, 
      ...
      if(!ifsimu){
        ...
        print(posterior$contact[n_simu])
...
if(F){
  save(fitlist, file = 'evoSSS/AB_capacity_m16.rdata')
  load('evoSSS/AB_capacity_m16.rdata')
}
...
if(F){
  ...
  if(F){
    save(fitlist, simu_Onset, plot_data, file = 'evoSSS/AB_capacity.rdata')
    load('evoSSS/AB_capacity.rdata')
  }
  ...
  pdf(paste0("Output/AB_capacity_m16.pdf"), width = 2.5, height = 1.6)
  print(p)
  dev.off()
}
...
p1 = ggplot() +
  ...
  scale_y_continuous(breaks = log2(c(250, 1000, 4000)),
                     labels = c(250, 1000, 4000)) +
  theme(panel.grid.minor = element_blank())

pdf(paste0("Output/AB_capacity_m16_par.pdf"), width = 2.6, height = 1)
print(p1)
dev.off()
