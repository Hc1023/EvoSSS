  ...
  for (j in 1:24) {
    if(cond) print(j)
    {
      ...
      if(cond){
        ...
        # Fit the model
        fit <- stan(file = 'evoSSS/capacity_beta.stan', data = stan_data, 
...
if(F){
  ...
  save(fitlist, file = 'evoSSS/AB_beta_m33.rdata')
}
...
if(F){
  ...
  if(F){
    save(fitlist, simu_Onset, plot_data, file = 'evoSSS/AB_beta_m33.rdata')
  ...
  pdf(paste0("Output/AB_beta_m33.pdf"), width = 2.5, height = 1.6)
  print(p)
  dev.off()
}
...
p2 = ggplot() +
  ...
  theme(panel.grid.minor = element_blank())
pdf(paste0("Output/AB_beta_m33_par.pdf"), width = 2.6, height = 1)
print(p1)
print(p2)
dev.off()
