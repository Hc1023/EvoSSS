...
if(F){
  save(fitlist, file = 'evoSSS/AB_capacity.rdata')
...
if(F){
  ...
  pdf(paste0("Output/AB_capacity.pdf"), width = 2.5, height = 1.8)
  print(p)
  dev.off()
}
...
p1 = ggplot() +
  ...
  scale_y_continuous(breaks = log2(c(500, 1000, 2000, 4000, 8000)),
                     labels = c(500, 1000, 2000, 4000, 8000)) +
  theme(panel.grid.minor = element_blank())

pdf(paste0("Output/AB_capacity_par.pdf"), width = 2.6, height = 1.2)
print(p1)
dev.off()
  ...
  ggplot() +
    geom_point(data = fexpect0, 
               aes(x = date, y = y, 
                   group = group, color = group)) +
    geom_line(data = data, 
              aes(x = date, y = y/40, group = group, color= group)) +
    scale_y_continuous(trans='log10') +
    coord_cartesian(ylim = c(2,max(data$y))) +
    labs(x = "Date", y = "Proportion") +
    theme_minimal() +
...
if(F){
  for (i in 1:length(fitlist)) {
    wd = paste0('Output/traceplot/AB_capacity_', as.character(i), '.png')
