...
full_dates <- as.Date('2020-05-01') + 1:885
...
observed_matrix = data.frame(v1 = merged_df[merged_df$V == 'D614G', 'count'],
                             ...
                             v4 = merged_df[merged_df$V == 'Omicron', 'count'])
...
determinant_fun = function(cond = T){
  ...
  Onsets_mat = observed_matrix[1:200,]
  # Generalized extraction of Onset columns for all variants
  Onsets <- list()
  for (i in 1:n) {
    Onsets[[i]] <- Onsets_mat[1:poolday, i] + 1e-10
  }
  ...
  expected_matrix[(nday - 29):nday,] = 0
  ...
  if(cond){
    stan_data <- list(
      ...
      pars_last = cbind(c(200, rep(0.3,4)), 
                        c(10, rep(0.01,4))),
  ...
  fexpect = data.frame(x = rep(1:nday,4), 
                       y = c(expected_matrix[1:nday,1],
                             expected_matrix[1:nday,2],
                             expected_matrix[1:nday,3],
                             expected_matrix[1:nday,4]),
                       group = factor(rep(c('D614G','Alpha','Delta','Omicron'), 
                                          each = nday),
                                      levels = c('D614G','Alpha','Delta','Omicron')))
  ...
  pars_last = cbind(c(mean(posterior$contact), 
                      mean(posterior$beta1),
                      mean(posterior$beta2), 
                      mean(posterior$beta3),
                      mean(posterior$beta4)),
                    c(sd(posterior$contact), 
                      sd(posterior$beta1),
                      sd(posterior$beta2), 
                      sd(posterior$beta3),
                      sd(posterior$beta4)))
  Onsets_mat = simu(seed_mats, 
                    N = seed_vec * pars_last[1,1] + 1, 
                    poolday, pars = pars_last[-1,1], n)
  ...
  for (j in 1:12) {
    ...
    j=16
    {
      ...
      expected_matrix[expected_matrix<10] = 10
...
data$group[data$color == '4'] = 'Omicron'
data$group = factor(data$group, levels = c('D614G','Alpha','Delta','Omicron'))
fexpect0 = data.frame(y = c(observed_matrix$v1,observed_matrix$v2,
                            observed_matrix$v3,observed_matrix$v4),
                      x = rep(1:nrow(observed_matrix),4),
                      group = rep(c('D614G', 'Alpha', 'Delta', 'Omicron'), each = nrow(observed_matrix)))
fexpect0$date = as.Date('2020-05-01') + fexpect0$x
fexpect0$group = factor(fexpect0$group, levels = c('D614G','Alpha','Delta','Omicron'))
ggplot() +
