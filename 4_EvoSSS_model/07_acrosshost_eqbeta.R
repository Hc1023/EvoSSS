...
if(F){
  ...
  # Fit the model
  fit <- stan(file = 'evoSIR_eqbeta.stan', data = stan_data, 
              iter = 15000, chains = 4, warmup = 10000,
              verbose = TRUE)
  print(fit)
  pdf(file = 'Output/evoSIR_eqbeta_trace.pdf', 
      width = 9, height = 2)
  traceplot(fit)
  dev.off()
  save(fit, file = 'evoSIR_eqbeta.rdata')
  return(fit)
}
...
p
pdf(paste0("Output/evoSIR_eqbeta.pdf"), width = 2.8, height = 1.8)
print(p)
dev.off()