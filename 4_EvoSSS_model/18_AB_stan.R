...
determinant_fun = function(cond = T, ifsimu  = T, n_simu = 1){
  ...
  if(!cond){
    seed_matrix[,1] = rmultinom(1, 6, c(0.4,0.6))
  }
...
if(F){
  save(plot_data, file = 'AB_plot.rdata')
  load('AB_plot.rdata')
}
...
pdf(paste0("Output/AB_plot.pdf"), width = 2.5, height = 1.8)
print(p)
dev.off()
...
pdf(paste0("Output/AB_plot_prevalence.pdf"), width = 2.5, height = 1.2)
print(p2)
dev.off()
