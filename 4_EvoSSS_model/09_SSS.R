...
simu <- function(seed_mat_I1, seed_mat_I2, N, dim, pars) {
  # Initial conditions
  ndays = dim*2
...
observed_days = nrow(observed_matrix)
stan_data <- list(
  dim = dim,
  cycle = cycle,
  poolday = poolday,
  ...
  contact_vec_init = contact_vec,
  simu_matrix = simu_matrix,
  observed_matrix = observed_matrix,
  pars = pars,
  Onsets_mat = Onsets_mat,
