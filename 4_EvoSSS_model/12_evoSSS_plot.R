...
if(T){
  ...
  if(F){
    save(plot_data_sampling, file = 'AB_constant_plot.rdata')
...
if(F){
  dfplot_simu = determinant_fun(ifsimu  = F, n_simu = 1, require_dfplot_simu = T)
...
if(F){
  data = determinant_fun(ifsimu  = F, n_simu = n_simu, require_dfplot_simu = F)
  data$date = as.Date('2019-12-31') + data$x
  data$group = 'A'
  data$group[data$color == '2'] = 'B'
  data$group = factor(data$group, levels = c('A','B'))
