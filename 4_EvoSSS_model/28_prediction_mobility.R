...
determinant_fun = function(cond = T, ifsimu  = T, n_simu = 1, kb, m,h){
  ...
  for (j in (thresh_cycle+1):all_cycle) {
    # print(j)
    {
      ...
      # Create seed_vec considering all variants
      seed_vec <- rowSums(sapply(Onsets, function(Onset) Onset[1])) %*% Mobility_matrix %>% as.numeric()
      ...
      pars_last = c(mean(posterior$contact)/m, 
                    mean(posterior$beta2)*kb,
                    mean(posterior$beta2))
      if(ifsimu){
        pars_last = c(posterior$contact[n_simu]/m, 
...
plot_data1 = getplot(kb = 1)
plot_data2 = getplot(kb = 1.1)
plot_data3 = getplot(kb = 0.9)
...
pdf(paste0("Output/prediction_AB_beta.pdf"), width = 4, height = 1.8)
print(p)
dev.off()


if(F){
  data = determinant_fun(cond = F, ifsimu  = F, n_simu = n_simu, kb = 0.9, m = 1, h =3)
