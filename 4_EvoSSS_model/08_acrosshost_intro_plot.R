...
delta = mean(time_intro2-time_intro1)
...
# Bar plot with error bars using ggplot2
p = ggplot(df, aes(x = group, y = delta, fill = delta)) +
  ...
  scale_fill_gradient2(low = "#7c615c", mid = "white", high = "#59788e", 
                       midpoint = 0, breaks = c(-10,0,10), labels = c('-10','0','+10'),
                       name = expression(Delta*d))
p
pdf(paste0("Output/R0_delta.pdf"), width = 4.5, height = 1)
print(p)
dev.off()
pdf(paste0("Output/R0_delta_legend.pdf"), width = 4.5, height = 2)
print(p)
dev.off()
library(ggplot2)
library(gridExtra)
...
p <- ggplot(data_melted, 
  ...
  geom_point(shape = 21) +
  scale_size_continuous(range = c(1, 12)) +  # Adjust size range based on your specific needs
  theme_minimal() +
  labs(x = "", y = "", size = "R0") +
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = 2.5, name = 'R0')


# Print the plot
print(p)

pdf(paste0("Output/R0.pdf"), width = 4, height = 2.2)
print(p)
dev.off()

pdf(paste0("Output/R0_legend.pdf"), width = 4.5, height = 2.5)
print(p)
dev.off()
