...
fe <- function(s = 3){
  ...
  for (t in 1:Tsss) {
    ...
    while (!success) {
      ...
      seed_mat_I1 = diag(seed_vec[1,])
      seed_mat_I2 = diag(seed_vec[2,])
      yh = simu(seed_mat_I1, seed_mat_I2, N, poolday)


      if (!is.null(nrow(yh))) {
        ph = sum(yh[,1])/sum(yh[,1] + yh[,2])
        if (!is.na(ph)) {
          success = 1
          ph_df = rbind(ph_df,
                        data.frame(ph = ph, 
                                   ysum = sum(yh[,1] + yh[,2])))
          seed_vec[,1] = rmultinom(1, s, c(ph, 1-ph)) 
        }
      }
    }
  }
...
library(ggpubr)
df1 = cyclefun(s = 20, n = 1000)
df2 = cyclefun(s = 6, n = 1000)
df3 = cyclefun(s = 3, n = 1000)
save(df1, df2, df3, file = 'bottleneck_transmission.rdata')

getplot = function(df){
  df$cycle = factor(df$cycle)
  values2 = c( '#b3b647', '#78b598','#67aab7','#9791ca')
  p = ggdensity(df[df$cycle%in%c('1','2','5','10'),], x = "ph", add = "mean",
            fill = 'cycle', color = 'cycle', rug = F) +
    ylab('') + xlab('') +
    geom_vline(xintercept = 0.3, linetype = "dashed") +
    scale_fill_manual(name = 'Cycle',
                      values = values2) +
...
p2 = getplot(df2)
p3 = getplot(df3)
pdf(file = paste0('Output/bottleneck_transmission.pdf'), width = 3, height = 1.5)
print(p1)
print(p2)
print(p3)
dev.off() 
